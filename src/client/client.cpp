...
void Client::join_game(int game_id)
{
	...
	if (!(client_socket->waitForBytesWritten(3000))) throw Errors(Errors::WRITE_SOCKET);

	//nacte si informace o hre a ulozi
	std::string game_info;

	if (client_socket->waitForReadyRead(3000)) read_from_socket(client_socket,game_info);
	else
	{ 
	...
	for (int i=0; i<this->height;i++)
		for (int j=0; j<this->width; i++)
			sscanf(game_info.c_str(),"%d",&(this->map[i][j]));
}
